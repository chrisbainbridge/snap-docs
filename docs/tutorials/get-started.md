...
Many Linux distributions, including Ubuntu, support _snap_ by default. You can check by running the `snap` command. If the snap command is not found, take a look at our {ref}`Installation guides <tutorials-install-the-daemon-index>` for further help.
...
Refer to {ref}`Managing updates <how-to-guides-work-with-snaps-manage-updates>` for more details.
...
For more details on where snaps store their data, see {ref}`Data locations <interfaces-data-locations>`.

## Create and restore a snapshot

A *snapshot* is a copy of the user, system and configuration data stored by *snapd* for one or more snaps on your system, and a snapshot of the data found in both `$HOME/snap/<snap-name>` and `/var/snap/<snap-name>` is stored in `/var/lib/snapd/snapshots/` (see {ref}`Data locations <interfaces-data-locations>` for more details).
...
See {ref}`Snapshots <how-to-guides-manage-snaps-create-data-snapshots>` for further details on creating, exporting, importing and validating snapshots.
...
For more details information on using snaps, see our {ref}`Snap How-to guides <how-to-guides-index>`.
